...
let Id_tema_1 =0;
function f_visualizar_subtemas_visualizados(Id_tema){
    Id_tema_1 = Id_tema;
    let Lote="";
    $.ajax({
            url: 'f_visualizar_subtemas_visualizados',
                            ...
                            for (let i = 0; i < subtemas.data.length; i++) {
                                if (subtemas.data[i][5] == 1) { 
                                    ...
                                    if (subtemas.data.length != i) {  
                                        ...
                                        document.getElementById('divPanelSubtemas').innerHTML = Lote;   
                                        Swal.fire({ title: "Señor funcionario...", text: "...tiene documentos por visualizar en esta sección.", type: "success"});
                                    ...
                                    if (subtemas.data.length != i) {  
                                        for (let k = 0; k < 1; k++) {
                                            Lote+=`<div class="col-lg-12 col-md-12 mt-3">
                                                <nav class="nav nav-pills flex-column flex-sm-row">
                                                    <a class="flex-sm-fill nav-link active" href="javascript:f_consulta_documento_visualizado(${subtemas.data[i][1]},${subtemas.data[i][2]})" style="background: linear-gradient(179deg, rgba(219, 223, 228) 25%, rgb(87, 125, 138) 62%) !important;">${subtemas.data[i][3]}</a>
                                                </nav>
                                            </div>`;
                                        }
                                        let divPanelSubtemas = document.getElementById('divPanelSubtemas');
...
function f_consulta_documento_visualizado(Id_tema,Id_subtema){
            let divPaneldocumnt="";
            $.ajax({
                url: 'visualiza_documento_aprobados',
        ...
        $.ajax({
            ...
            type: 'POST',
                success: function (response) {
                    ...
                    if(Rta.status == 'OK'){
                        f_visualizar_subtemas_visualizados(Id_tema_1);
                        Id_tema_1 =0;
                        ...
                        let t=setTimeout("redirecciona()",5500); 
                        return true;
                    }else{
...
function redirecciona(){
    ...
    location.href ="/documentos_generados"; 
}
...
//     $('#btn_devolver_documento').click(function() 
    ...
    function f_devolver_aprobado_visualizado(){
        let Mensaje_revisor = $('#Mensaje_revisor').val()
        $.ajax({
            url: 'p_devolver_documento_visualizado',
