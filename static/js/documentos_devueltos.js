...
let Id_tema_1 =0;
function f_visualizar_subtemas_devueltos(Id_tema){
    Id_tema_1 = Id_tema;
    let Lote="";
    $.ajax({
            url: 'f_visualizar_subtemas_devueltos',
                            ...
                            for (let i = 0; i < subtemas.data.length; i++) {
                                if (subtemas.data[i][5] == 1) { 
                                    ...
                                    if (subtemas.data.length != i) {  
                                        for (let k = 0; k < 1; k++) {
                                            Lote+=`<div class="col-lg-12 col-md-12 mt-3">
                                                <nav class="nav nav-pills flex-column flex-sm-row">
                                                    <a class="flex-sm-fill nav-link active" href="javascript:f_consulta_documento(${subtemas.data[i][1]},${subtemas.data[i][2]})" style="background: linear-gradient(179deg, rgba(87, 16, 3) 25%, rgb(134, 137, 138) 62%) !important;">${subtemas.data[i][3]}&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp${subtemas.data[i][4]}</a>
                                                </nav>
                                            </div>`;
                                        }
                                        ...
                                        document.getElementById('divPanelSubtemas').innerHTML = Lote; 
                                        Swal.fire({ title: "Señor funcionario...", text: "...tiene documentos rechazados en esta sección.", type: "success"});
