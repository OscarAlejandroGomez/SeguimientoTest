...
let Id_tema_1 =0;
function f_visualizar_subtemas(Id_tema){
    Id_tema_1 = Id_tema;
    let Lote="";
    $.ajax({
            url: 'f_visualizar_subtemas',
            data: {Id_tema :Id_tema },
            type: 'POST',
                success: function (response) {
                    var subtemas = JSON.parse(response);
                    if (subtemas.status == 0){  
                        let Lote="";
                            ...
                            for (let i = 0; i < subtemas.data.length; i++) {
                                if (subtemas.data[i][5] == 1) { 
                                    ...
                                    if (subtemas.data.length != i) {  
                                        ...
                                        document.getElementById('divPanelSubtemas').innerHTML = Lote; 
                                        Swal.fire({ title: "Señor funcionario...", text: "...tiene documentos por revisar en esta sección.", type: "success"});
